...
class NavDP_RGBD_Backbone(nn.Module):
    ...
    def forward(self, images, depths):
        ...
        with torch.no_grad():  # RGB encoder 冻结
            # 获取模型实际所在的设备（而不是 self.device，因为模型可能被移动到其他设备）
            model_device = next(self.rgb_model.parameters()).device
            
            # ===== 处理 RGB =====
            if len(images.shape) == 4:
                # 单帧模式（未使用）
                tensor_images = torch.as_tensor(images, dtype=torch.float32, device=model_device).permute(0, 3, 1, 2)
                tensor_images = tensor_images.reshape(-1, 3, self.image_size, self.image_size)
                tensor_norm_images = (tensor_images - self.preprocess_mean.reshape(1, 3, 1, 1).to(model_device)) / \
                                     self.preprocess_std.reshape(1, 3, 1, 1).to(model_device)
                image_token = self.rgb_model.get_intermediate_layers(tensor_norm_images)[0]
            
            elif len(images.shape) == 5:
                # 多帧模式（主要使用）
                tensor_images = torch.as_tensor(images, dtype=torch.float32, device=model_device).permute(0, 1, 4, 2, 3)
            ...
            if len(depths.shape) == 4:
                # 单帧模式
                tensor_depths = torch.as_tensor(depths, dtype=torch.float32, device=model_device).permute(0, 3, 1, 2)
                ...
                # 多帧模式
                tensor_depths = torch.as_tensor(depths, dtype=torch.float32, device=model_device).permute(0, 1, 4, 2, 3)
            ...
            former_query = self.former_query(
                torch.zeros((image_token.shape[0], self.memory_size * 16, 384), device=model_device)
            )
            # Query: (batch, 128, 384)
            # 128 = 8帧 × 16个query/帧
...
class NavDP_ImageGoal_Backbone(nn.Module):
    ...
    def forward(self, images):
        ...
        with torch.no_grad():
            ...
            # 获取模型实际所在的设备
            model_device = next(self.imagegoal_encoder.parameters()).device
...
class NavDP_PixelGoal_Backbone(nn.Module):
    ...
    def forward(self, images):
        ...
        with torch.no_grad():
            ...
            # 获取模型实际所在的设备
            model_device = next(self.pixelgoal_encoder.parameters()).device
