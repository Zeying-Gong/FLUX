"""
CFM Agent 类（NavDP 的 CFM 版本）
负责数据预处理、历史帧管理、调用 CFM 策略网络推理
"""
import torch
import numpy as np
import cv2
...
class CFM_Agent:
    """
    CFM 导航代理
    与 NavDP 相同的预处理与流程，策略为 CFM ODE 采样（可配置步数）
    """
    def __init__(self,
                 ...
                 heads=8,
                 token_dim=384,
                 navi_model="./100.ckpt",  # CFM 微调权重路径（.ckpt 或 pytorch_model.bin）
                 normalization_config=None,  # 归一化配置 JSON（与训练一致）
                 cfm_num_steps=5,       # ODE 积分步数（默认 5 步，保持原始测评逻辑）
                 device='cuda:0'):
        ...
        self.navi_former.load_state_dict(ckpt, strict=False)
