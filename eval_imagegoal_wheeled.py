"""
Image-goal navigation evaluation in Isaac Sim.

Async planning thread calls the NavDP server with goal images; MPC tracks the
planned trajectory. Records success, SPL, and distance.
"""
import argparse
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="Image-goal navigation evaluation")
parser.add_argument(
...
def planning_thread(env, camera_intrinsic):
    ...
    while not stop_event.is_set():
        try:
            ...
            trajectory_points_camera, all_trajectories_camera, all_values_camera = imagegoal_step(
        ...
        if current_trajectory is not None:
            ...
            for i in range(args_cli.num_envs):
                vis_image = vis_manager[i].visualize_trajectory(
                    np.concatenate((images[i], goal_images[i]), axis=1),
                    depths[i][:, :, None],
                    camera_intrinsic.cpu().numpy(),
                ...
                try:
                    ...
                    vis_image = draw_box_with_text(
                        ...
                        "point goal:(%.2f, %.2f)" % (goal_poses[i][0], goal_poses[i][1])
                    )
                    cv2.imwrite(
                        f"frame_{algo}_imagegoal_{scene_name}.png",
