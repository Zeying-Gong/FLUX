"""
Dynamic exploration evaluation with simulated people in Isaac Sim.

Runs NavDP (no-goal) planning, MPC, occupancy exploration metrics, and social
navigation metrics across scripted episodes (JSON per episode).
"""
import argparse

parser = argparse.ArgumentParser(description="Dynamic exploration with people")
parser.add_argument(
...
from utils_tasks.exploration_utils import update_occupancy
...
def planning_thread(env, camera_intrinsic):
    """Background thread: NavDP nogoal planning, world-frame trajectories, MPC state."""
    global mpc
    while not stop_event.is_set():
        try:
            ...
            trajectory_points_camera, all_trajectories_camera, all_values_camera = nogoal_step(
                image, depth, port=args_cli.port
            )
            ...
            with torch.inference_mode():
                ...
                if current_trajectory is not None:
                    ...
                    for i in range(args_cli.num_envs):
                        ...
                        try:
                            vis_image = draw_box_with_text(vis_image, 0, 0, 430, 50, f"cmd lin.:{v:.2f} ang.:{w:.2f}")
                            ...
                            if current_all_values is not None:
                                vis_image = draw_box_with_text(
                                    ...
                                    f"critic max:{np.max(current_all_values[i]):.2f} min:{np.min(current_all_values[i]):.2f}"
                                )
                            vis_image = draw_box_with_text(vis_image, 0, 820, 430, 50, f"explore area:{explore_areas[i]:.2f} sq m")
                            if frame_count > 0:
