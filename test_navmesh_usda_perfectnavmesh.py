...
def add_exclude_volumes_from_semantic_map(stage, semantic_map_json,
    ...
    for it in sample_items:
        ...
        cx_sm = 0.5*(x_l+x_r)
        cy_sm = 0.5*(y_b+y_t)
        cx_is, cy_is = sm_to_isaac(cx_sm, cy_sm)
        print(f"[Exclude] {it['item_id']}: sm=({cx_sm:.3f},{cy_sm:.3f}) "
              f"→ isaac=({cx_is:.3f},{cy_is:.3f})")
    ...
    for item in items:
        ...
        # bbox 四个角都变换，取变换后的 min/max（因为 flip+negate 可能改变大小关系）
...
def try_bake_navmesh():
    ...
    if ARGS.semantic_map_json and os.path.exists(ARGS.semantic_map_json):
        add_exclude_volumes_from_semantic_map(
            stage, ARGS.semantic_map_json, ARGS.exclude_labels,
        )
