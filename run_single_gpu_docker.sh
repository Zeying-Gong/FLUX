#!/bin/bash
# run_single_gpu_docker.sh – 在 Docker 容器内执行单 GPU 数据采集
# 用法: run_single_gpu_docker.sh <GPU_ID> <START_IDX> <END_IDX>
...
SCRIPT_PY="/workspace/FLUX/sage_utils/tracking_episode_collection.py"
PYTHON="/isaac-sim/python.sh"
ALL_SCENES=($(ls "$SCENE_DIR"))

LOG_DIR="/workspace/FLUX/logs_formal_v2"
LOG_NAME="collect_${ROBOT_ONLY:-all}_${CAM_ONLY:-all}_$(date '+%Y%m%d_%H%M%S')_gpu${GPU}"
...
for combo in $COMBOS; do
  ...
  for ((i=S; i<E; i++)); do
    scene="${ALL_SCENES[$i]}"
    OUT_DIR="$LOG_DIR/${robot}_${cam}/$scene"
    ...
    for ((batch_start=0; batch_start<NUM_EPISODES; batch_start+=BATCH_SIZE)); do
      ...
      for ((attempt=0; attempt<MAX_RETRIES; attempt++)); do
        ...
        $PYTHON "$SCRIPT_PY" \
          ...
          --early_abort_tracking_rate 0.3 \
          --early_abort_min_steps 30 \
          --min_tracking_rate 0.3 \
          --resume \
