...
dual_sys_eval_url = None
def display_image_from_bytesio(image_bytesio):
    """
    使用OpenCV显示io.BytesIO对象中的图像
    """
    # 从BytesIO对象读取数据
    ...
    # 显示图像
    success = cv2.imwrite('./check.jpeg', image, [cv2.IMWRITE_JPEG_QUALITY, 95])
    print("图像已显示，按任意键关闭窗口...")
    
    return True

def dual_sys_eval(image_bytes, depth_bytes, front_image_bytes):
    # print(image_bytes)
    global policy_init, http_idx, first_running_time,robot_orientation
    data = {"reset": policy_init, "idx": http_idx}
    json_data = json.dumps(data)
    
    # display_image_from_bytesio(image_bytes)
    ...
    response = requests.post(dual_sys_eval_url, files=files, data={'json': json_data, 'robot_orientation':json.dumps(robot_orientation)}, timeout=100)
    ...
    while True:
        ...
        if current_control_mode == ControlMode.MPC_Mode:
            ...
            if mpc is not None and manager is not None and odom is not None:
                ...
                manager.move(v, 0.0, w) #manager.move(v*3, 0.0, w*3) for g1
...
class Go2Manager(Node):
    ...
    def odom_callback(self, msg):
        ...
        robot_orientation = [msg.pose.pose.position.x, msg.pose.pose.position.y, yaw, msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.angular.z]
...
if __name__ == '__main__':
    ...
    dual_sys_eval_url = args.server_url
    print(dual_sys_eval_url)
