...
class VisualizationManager:
    ...
    def visualize_trajectory(self, rgb_image, depth_image, intrinsic, trajectory_points, robot_pose, camera_roll=0, all_trajectories_points=None, all_trajectories_values=None, goal_pos=None):
        ...
        # Concatenate images
        # print("rgb_image shape", rgb_image.shape, "vis_resized shape",vis_resized.shape)
        ...
        if goal_pos is not None:
            ...
            cos_yaw = np.cos(yaw)
            sin_yaw = np.sin(yaw)
            current_rotation = np.array([[cos_yaw, -sin_yaw], [sin_yaw, cos_yaw]])
            transformed_points = (current_rotation @ np.vstack([dx, dy])).T
            ...
            # print("type(grid_points)",type(grid_points),)
            # Convert to visualization coordinates (adjust for image coordinate system)
            if 0 not in grid_points.shape:
                ...
                cv2.circle(vis_image, tuple(vis_points[0]), 3, (255, 255, 255), -1, cv2.LINE_AA) 
        ...
        def value_to_color(value, values_min=None, values_max=None):
            # Normalize value to [0, 1] based on a fixed range [-1.2, 0.2]
            fixed_min = -1.2
            fixed_max = 0.2
            ...
            # Get the colormap
            colormap = cm.get_cmap('RdYlGn')  # 使用红到绿的色图

            # Get the color from the colormap
            color = colormap(normalized)  # 获取 RGBA 颜色值

            # Return the BGR color (multiply by 255 and convert to int)
            return (int(color[0] * 255), int(color[1] * 255), int(color[2] * 255))  # 返回 BGR 格式的颜色
        ...
        vis_resized_all = cv2.resize(vis_image_all, (int(rgb_image.shape[0]), int(rgb_image.shape[0])), interpolation=cv2.INTER_CUBIC)
        # Apply slight Gaussian blur to smooth pixelated edges
        vis_resized_all = cv2.GaussianBlur(vis_resized_all, (3, 3), 0.5)
        
    
        
        return vis_resized, vis_resized_all
