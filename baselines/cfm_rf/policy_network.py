"""
Linear-path Rectified Flow 推理网络（独立版，不依赖 cfm/）

原 cfm_rf/policy_network.py 动态加载 cfm/policy_network.py 作为基类，
本文件将两者合并，可直接删除 cfm/ 文件夹。

与原始 cfm/policy_network.py 的核心差异（RF 版本的修改点）：
1. cond_pos_embed: memory_size*16 + 4 = 132（原 cfm 也是 +4，一致）
2. sample_ode_euler: 使用 torch.round 将连续 t 映射到整数 k（匹配训练离散 timesteps）
3. predict_*_action: denormalize + /4.0 + cumsum（训练启用了归一化）
"""
...
class CFM_Policy(nn.Module):
    ...
    与旧 cfm/ 版本的唯一运行时区别：
      sample_ode_euler 内 k = round(t * (T-1))，保证整数离散步。
    """
    ...
    def sample_ode_euler(self, goal_embed, rgbd_embed, sample_num, num_steps=5):
        ...
        关键：连续 t 映射到整数 k = round(t * (T-1))，
        保证与训练时的离散 timestep 对齐。

        num_steps=5  → 5 次前向（默认，效率与旧 CFM 一致）
        num_steps=9  → 9 次前向（与训练 k 值完全对齐）
        """
        ...
        for i in range(num_steps):
            t_cur = t_seq[i]
            dt = t_seq[i] - t_seq[i + 1]  # 正值
            # RF 修改点：round 到最近整数，clamp 到训练范围 [1, T-1]
