"""
Exploration (no explicit goal) navigation evaluation in Isaac Sim.

Uses an async planning thread (NavDP server), MPC tracking, and occupancy-based
exploration area from a metric camera. Records time and explored area per episode.
"""
import argparse
from isaaclab.app import AppLauncher

parser = argparse.ArgumentParser(description="Exploration navigation evaluation")
parser.add_argument(
...
def planning_thread(env, camera_intrinsic):
    """Background thread: call NavDP, transform trajectories to world frame, update MPC."""
    global mpc
        ...
        if current_trajectory is not None:
            ...
            for i in range(args_cli.num_envs):
                ...
                try:
                    ...
                    cv2.imwrite(
                        f"frame_{algo}_nogoal_{scene_name}.png",
        ...
        for i in range(args_cli.num_envs):
            if dones[i] == True:
                ...
                evaluation_metrics.append({
                    'time': episode_steps[i] * env.unwrapped.step_dt,
