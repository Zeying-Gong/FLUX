...
# local_isaaclab_path = "/workspace/IsaacLab/source"
# if local_isaaclab_path not in sys.path:
#     sys.path.insert(0, local_isaaclab_path)
...
def cleanup_simulation(env=None, simulation_app=None):
    ...
    try:
        ...
        # 3. 提前清理 camera sensors (关键步骤)
        if env is not None:
            print("  Detaching camera sensors...")
            try:
                # 访问 unwrapped env 来获取 scene
                ...
                if hasattr(unwrapped_env, 'scene') and hasattr(unwrapped_env.scene, 'sensors'):
                    ...
                    if camera_sensor is not None:
                        # 手动 detach annotators，避免在 __del__ 时出错
