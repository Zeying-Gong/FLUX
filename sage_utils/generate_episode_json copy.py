...
# Sampling helpers
# ═══════════════════════════════════════════════════════════════════════════
MIN_ROBOT_DIST   = 5.0    # minimum robot start→goal distance (m)
MAX_ROBOT_DIST   = 20.0
MIN_CHAR_DIST    = 5.0    # character must be >= this far from robot endpoints
MIN_CHAR_SPACING = 5.0    # between characters and robot
...
def main() -> int:
    ...
    def sample_point():
        px_py = _sample_free_point_2d(grid, esdf, rng, min_clearance_m=ARGS.robot_radius_2d * 1.5)
        if px_py is None:
            return None
        # 直接用随机采样的点，不再 snap 到 ESDF 最大值
    ...
    for i, pos in enumerate(char_positions):
        ...
        for wi in range(ARGS.num_waypoints):
            for _ in range(200):
                ...
                if target is None:
                    continue
                # 先用欧氏距离粗筛，太近的直接跳过（省去 A* 计算）
                if _dist2(target, prev) < 2.0:
                    continue
                path_pts = plan_path(prev, target)
                if path_pts is None:
                    continue
                # 用测地距离精确判断
                if _path_length(path_pts) < MIN_CHAR_DIST:
                    continue
                # ★ NavMesh 连通性验证（验证 target 和 robot_start 连通，
                #   保证任意两个 episode 点都在同一连通分量）
                ...
                print(f"    GoTo#{wi}: {len(path_pts)} path points, "
                      f"geodesic={geo_dist:.1f}m → ({x:.2f},{y:.2f})")
                prev = target
                break
            else:
                print(f"    GoTo#{wi}: failed after 200 attempts, skipping")
