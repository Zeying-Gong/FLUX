...
# If the target has been nearly stationary for DONE_SUSTAINED_STEPS control steps,
# we assume all its commands have finished.  Then we wait DONE_GRACE_STEPS more
# steps (≈ grace_s seconds) so the robot has time to catch up before ending.
DONE_TGT_SPD_THRESH   = 0.15   # m/s below which target counts as "stopped"
DONE_SUSTAINED_STEPS  = 25     # consecutive stopped steps to declare done
DONE_GRACE_STEPS      = 60     # extra steps after done before episode ends (~6 s)
...
def setup_episode_full(
    ...
    # Phase 4: Attach behavior scripts — robot and characters start together.
...
def main() -> int:
    ...
    occ = None
    if ARGS.semantic_map_json:
        occ = _load_occ_grid(ARGS.semantic_map_json, ARGS.occ_scale, ARGS.robot_radius_2d)
    global _GLOBAL_OCC
    ...
    for ep_idx, ep_path in enumerate(selected):
        ...
        if is_first:
            ...
            # Phase 4: Attach behavior scripts — robot and characters start together.
            print("[Episode 0] Phase 4: attaching behavior scripts (robot+chars start together)...")
            attach_behavior_scripts_to_characters(simulation_app)
        else:
        ...
        while step < ARGS.max_steps:
            ...
            if len(_TARGET_POS_HIST) >= 3:
                ...
                _app  = float(np.dot(_tv, _to_r / max(float(np.linalg.norm(_to_r)), 0.1)))
                _tgt_vel_dbg = f" tgt_v={_tv_spd:.2f}m/s appr={_app:+.2f}m/s"
            ...
            # ── Character-done detection ──────────────────────────────────
            # Estimate current target speed from recent position history.
            _tgt_v_now = 0.0
            if len(_TARGET_POS_HIST) >= 3:
                _sdt2 = ARGS.physics_dt * ARGS.decimation
                _tv2  = (_TARGET_POS_HIST[-1] - _TARGET_POS_HIST[-3]) / (2 * _sdt2)
                _tgt_v_now = float(np.linalg.norm(_tv2))
            if _tgt_v_now < DONE_TGT_SPD_THRESH:
                _tgt_stationary_cnt += 1
            else:
                _tgt_stationary_cnt = 0
            if not _tgt_done and _tgt_stationary_cnt >= DONE_SUSTAINED_STEPS:
                _tgt_done      = True
                _tgt_done_step = step
                print(f"[EP{ep_id}] step={step} TARGET DONE "
                      f"(stationary >{DONE_SUSTAINED_STEPS} steps, "
                      f"tgt_v={_tgt_v_now:.3f}m/s, dist={dist_to_target:.2f}m)")
            ...
            if in_window:
                ...
                # Keeps robot at ideal visual distance even while rotating.
