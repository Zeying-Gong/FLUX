    ...
    if clothing_profiles is not None:
        ...
        try:
            appearance_block = assign_episode_appearance(
                clothing_profiles, episode_id, seed, char_names)
...
def main():
    ...
    # For TRACKING, every requested episode must succeed; any failure
    # gets surfaced via a non-zero exit code so the batch runner counts
    # the scene as FAIL (rather than silently writing 0 episodes).
