...
debug_sampling.py
─────────────────────────────────────────────────────────────────────────────
Standalone debug script: sample robot + pedestrian spawn/waypoints from 2D semantic map,
no Isaac Sim / NavMesh dependency; NavMesh connectivity check uses A* stub.
...
Usage:
    python debug_sampling.py \
        ...
        --episode_ids 0 1 2 \
        --num_people 3 \
        --vis                    # generate visualization PNG

Once debugged, paste generate_spawn() logic back into the pipeline.
─────────────────────────────────────────────────────────────────────────────
"""
...
# ══════════════════════════════════════════════════════════════════════
# Constants  (keep in sync with pipeline)
# ══════════════════════════════════════════════════════════════════════
MIN_ROBOT_DIST              = 5.0
MAX_ROBOT_DIST              = 20.0
MIN_CHAR_DIST               = 3.0   # min dist from spawn to robot_start
MIN_CHAR_SPACING            = 3.0   # min spacing between pedestrians
MIN_CHAR_GOAL_TO_ROBOT_GOAL = 3.0   # min dist from last ped waypoint to robot_goal
MIN_ISLAND_AREA_THRESHOLD   = 6.0   # ignore islands smaller than this area
MIN_REACHABLE_PX            = 100
NUM_CHAIN_WPT               = 2     # waypoints per pedestrian after spawn

# -- SMAP weights --
SMAP_W_ESDF        = 0.30   # static obstacle safety score weight
SMAP_W_PSDF        = 0.45   # pedestrian safety score weight
SMAP_W_EFFICIENCY  = 0.25   # efficiency (dist-to-goal) score weight

# PSDF: dual-layer Proxemics Gaussian (ref. Hall 1966 proxemics zones)
PSDF_INNER_R       = 0.45   # personal space inner radius (m), strong penalty
PSDF_OUTER_R       = 1.2    # social space outer radius (m), weak penalty
PSDF_INNER_W       = 3.0    # inner Gaussian peak weight
PSDF_OUTER_W       = 1.0    # outer Gaussian peak weight

# ref paths: diversity via penalty map
# After each path, add cost near its pixels to force next path onto different corridors
REF_N_PATHS        = 3       # number of ref paths to plan
REF_BLOCK_SIGMA_M  = 0.4     # block bandwidth (m): narrower=closer paths, wider=more diverse
REF_BLOCK_WEIGHT   = 8.0     # block penalty strength (larger = more forced detour)
REF_COLORS         = ["#FF6B6B", "#FFD700", "#00FF88"]   # red=safe, gold=mid, green=eff
REF_LABELS         = ["ref_1 (eff)", "ref_2 (mid)", "ref_3 (safe)"]
# cost weights for each ref path: (esdf_w, psdf_w)
# ref_1 (red):    high psdf_w → strongly avoids ped zones, path may be longer
# ref_2 (gold):   balanced esdf/psdf
# ref_3 (green):  very low weights → near-shortest-path, may pass through ped zones
REF_PATH_WEIGHTS   = [
    (0.15, 1.5),    # efficient: nearly shortest path   → red
    (0.10, 0.5),    # mid:       balanced               → gold
    (0.05, 0.05),   # safe:      strong ped avoidance  → green
]

# pedestrian path: ESDF wall-avoidance weight
PED_ESDF_WEIGHT            = 0.08
# robot path: ESDF wall-avoidance weight
ROBOT_ESDF_WEIGHT          = 0.12
# robot path: pedestrian repulsion field params
ROBOT_PED_REPULSION_SIGMA  = 1.5
ROBOT_PED_REPULSION_WEIGHT = 2.0

# actual_num = floor(nav_area / AREA_PER_PERSON_M2), capped by num_people
AREA_PER_PERSON_M2          = 5.0
# island allocation: min area per person per island
...
def astar(grid, start_px, goal_px, esdf=None, min_corridor_m=0.0,
    ...
    """
    esdf_weight > 0: add ESDF reciprocal to move cost, path actively avoids walls.
    extra_cost: extra cost map of shape (H,W) to overlay (e.g. ped repulsion field).
    """
    H, W = grid.shape
...
def plan_island_allocation(actual_num, robot_island_idx, valid_islands):
    """
    Island allocation strategy:
      - First person always assigned to robot_island (guarantees at least 1 same-island ped)
      - Remaining (actual_num-1) distributed by island area ratio (largest-remainder rounding)
      - Each island cap = max(1, floor(area / MIN_ISLAND_AREA_PER_PERSON))
      - If total still < actual_num after capping, fill remaining by area descending
      - Returns list of exactly actual_num island indices
    """
    if actual_num <= 0:
        return []
    if actual_num == 1:
    ...
    # ensure robot_island has at least 1 person (may be 0 after proportional allocation)
    if counts[robot_island_idx] == 0:
        # borrow 1 from the island with most allocations
    ...
    # expand to list: robot_island's first slot comes first
    allocation = [robot_island_idx]
    remaining_counts = counts[:]
    remaining_counts[robot_island_idx] -= 1   # already placed one
    # expand remaining by area desc (larger islands first for higher priority sampling)
    order = sorted(range(n_isl), key=lambda k: -valid_islands[k]["area_m2"])
    for idx in order:
...
# ══════════════════════════════════════════════════════════════════════
# Stub: NavMesh connectivity (no Isaac Sim; use A* instead)
# ══════════════════════════════════════════════════════════════════════
def navmesh_connected_stub(plan_fn, a, b) -> bool:
    """Use A* path planning as a stub for NavMesh connectivity check."""
...
def generate_spawn(
    ...
    """
    Core sampling function (pure 2D, no Isaac Sim dependency).
    Returns dict with:
    ...
    def plan_path(a, b, corridor_m=None, esdf_weight=0.0, extra_cost=None):
        """
        corridor_m=None  -> robot default: robot_radius*0.5
        esdf_weight      -> ESDF wall-avoidance cost strength
        extra_cost       -> additional cost map to overlay (e.g. ped repulsion), shape=(H,W)
        """
    ...
    def plan_path_ped(a, b):
        """Pedestrian path: no corridor constraint, ESDF cost steers path away from walls."""
        return plan_path(a, b, corridor_m=0.0, esdf_weight=PED_ESDF_WEIGHT)

    def build_ped_repulsion(spawn_list, sigma_m=2.0, weight=3.0):
        """
        Add Gaussian repulsion field around spawn_list pixel positions.
        sigma_m: influence radius (m); weight: peak cost.
        Returns float32 cost map of shape (H,W).
        """
        H, W = grid.shape
        ...
        for pos in spawn_list:
            ...
            # compute within limited window to avoid full-map scan
            r = int(sigma_px * 3)
    ...
    # -- robot sampling (3-level fallback) --
    ...
    for attempt in range(200):
        ...
        msg = (f"  [L1 OK attempt={attempt+1}] "
               ...
               f"island={s_island} area={valid_islands[s_island]['area_m2']:.1f}m2 "
    ...
    # Level 3: largest island, drop distance constraint
    if robot_start is None:
    ...
    for i in range(actual_num):
        ...
        same_island   = (target_island == robot_island_idx)  # whether connectivity check is needed
        placed = False
        ...
        for attempt in range(200):
            ...
            # same_island peds need min dist from robot_start; non-same_island peds do not
            ...
            # only same_island peds need connectivity check to robot_start
            ...
            # waypoint chain: store (waypoint, path_to_waypoint) pairs
            chain = [(p0, [])]   # spawn point has no incoming path
            ...
            for wpt_i in range(NUM_CHAIN_WPT):
                ...
                for _ in range(200):
                    ...
                    # non-same_island peds: waypoints must stay within their own island
            ...
            # endpoint swap: if last waypoint too close to robot_goal,
            #   swap with the waypoint farthest from robot_goal and re-plan affected segments.
            # chain[0]=spawn (fixed), chain[1:]=waypoints
            wpts = chain[1:]   # list of (pos, path_from_prev)
            if len(wpts) >= 2 and dist2(wpts[-1][0], robot_goal) < MIN_CHAR_GOAL_TO_ROBOT_GOAL:
                # find candidate farthest from robot_goal (excluding last itself)
        ...
        if not placed:
            ...
            # Fallback: relax distances; must_same only for ped0 (must stay on robot island)
            ...
            for attempt in range(200):
                ...
                if not must_same and target_island is not None and p0_island != target_island:
                    fb_fail["wrong_island"] = fb_fail.get("wrong_island",0)+1; continue
                # only same_island peds check connectivity
                if same_island and plan_path_ped(p0, robot_start) is None:
    ...
    # -- robot path planning (after ped sampling, overlay ped repulsion field) --────
    ...
    if robot_path is None:
        # fallback: drop ped repulsion, use ESDF cost only
        robot_path = plan_path(robot_start, robot_goal,
                               esdf_weight=ROBOT_ESDF_WEIGHT)
        log.append("  [robot_path] fallback: ped repulsion dropped, ESDF-only")
    if robot_path is None:
        robot_path = plan_path(robot_start, robot_goal)
...
def build_psdf(grid, scale, ped_all_pts_px):
    """
    PSDF: overlay dual-layer Proxemics Gaussian danger field over all pixels on pedestrian paths.
    ped_all_pts_px: list of (px, py), all pixels on ped paths (chain points + intermediate path points)
    Returns psdf_danger (H,W) in [0, ~), higher=more dangerous;
    and psdf_score (H,W) = exp(-danger), [0,1], higher=safer.
    """
    H, W = grid.shape
    ...
    for cx, cy in ped_all_pts_px:
        ...
        for dy in range(-r_max, r_max+1):
            for dx in range(-r_max, r_max+1):
                ...
                if grid[ny, nx] == 1: continue
                d = math.hypot(dx, dy)
                # dual-layer Proxemics Gaussian (ref. Hall 1966)
    ...
    # normalize danger field to safety score
    danger[grid == 1] = 0.0
    max_d = danger.max() + 1e-6
    # score: 1 far from peds, 0 at ped location
...
def build_smap(grid, esdf, scale,
               ped_paths_px,     # list of list of (px,py): full path pixels per pedestrian
               robot_goal_px):
    """
    Returns smap, esdf_score, psdf_danger, psdf_score, eff_score
    """
    H, W = grid.shape
    ...
    # -- 2. PSDF (dual-layer Proxemics Gaussian over full ped path pixels) --
    all_ped_px = [pt for path in ped_paths_px for pt in path]
    psdf_danger, psdf_score = build_psdf(grid, scale, all_ped_px)

    # -- 3. Efficiency score (full-map distance-to-goal field) --
...
def plan_diverse_ref_paths(grid, esdf, smap, start_px, goal_px, scale, n=3):
    """
    Plan n semantically distinct reference paths:
      - Each path has its own (esdf_w, psdf_w) from REF_PATH_WEIGHTS
      - After each path is planned, add a soft Gaussian penalty along it
        so subsequent paths are nudged away (diversity without hard blocking)
    Returns list of path_px (list of (px,py)) or None.
    """
    H, W = grid.shape
    block_sigma_px = REF_BLOCK_SIGMA_M / scale
    r_block        = int(block_sigma_px * 3) + 1
    diversity_penalty = np.zeros((H, W), dtype=np.float32)

    paths = []
    for k in range(min(n, len(REF_PATH_WEIGHTS))):
        esdf_w, psdf_w = REF_PATH_WEIGHTS[k]
        ...
        while open_set:
            ...
            for d in dirs:
                ...
                step      = math.hypot(d[0], d[1])
                clearance = max(esdf[ny, nx], 1e-3)
                psdf_s    = float(smap[ny, nx])   # use SMAP as proxy for ped safety
                # semantic cost: safe path pays more to be near walls/peds
                cost = (step
                        + esdf_w / clearance
                        + psdf_w * (1.0 - psdf_s)
                        + float(diversity_penalty[ny, nx]))
                tg = g_cost[cur] + cost
        ...
        if found is None:
            paths.append(None)
            print(f"  [REF] path {k+1} ({REF_LABELS[k]}): not found")
            continue
        ...
        path = path[::-1]
        paths.append(path)
        print(f"  [REF] path {k+1} ({REF_LABELS[k]}): {len(path)} pts  "
              f"esdf_w={esdf_w} psdf_w={psdf_w}")

        # Add soft diversity penalty along this path (lighter than before)
        for px, py in path:
            for dy in range(-r_block, r_block+1):
                for dx in range(-r_block, r_block+1):
                    ...
                    if 0<=nx<W and 0<=ny<H and grid[ny,nx]==0:
                        d = math.hypot(dx, dy)
                        diversity_penalty[ny, nx] += (
                            REF_BLOCK_WEIGHT * 0.3   # softer than hard blocking
                            * math.exp(-0.5*(d/block_sigma_px)**2))
...
def visualize(result, grid, esdf, min_x, min_y, scale,
    ...
    # -- convert pedestrian full paths to pixel coordinates --
    # paths[wi]: A* path from chain[wi-1]->chain[wi] (Isaac coords list)
    # include chain points + all intermediate path points in PSDF
    ped_paths_px = []   # one list of (px,py) per pedestrian
    for ped in result.get("pedestrians", []):
        ...
        # chain[wi] and intermediate path points
    ...
    # -- build SMAP and sub-fields --
    smap, esdf_score, psdf_danger, psdf_score, eff_score = build_smap(
        grid, esdf, scale, ped_paths_px, goal_px)

    # -- plan diverse reference paths --
    ref_paths_px = plan_diverse_ref_paths(
        grid, esdf, smap, start_px, goal_px, scale, n=REF_N_PATHS)

    # -- figure layout: 2x2 --
    # ax_occ  : occupancy + ped paths (debug base)
    # ax_esdf : ESDF (static obstacle clearance)
    # ax_psdf : PSDF danger field (Proxemics)
    # ax_smap : SMAP heatmap + diverse ref paths
    ...
    def draw_ref_paths(ax):
        for k, rp in enumerate(ref_paths_px):
            if rp and len(rp) >= 2:
                ...
                # sample SMAP score dots along path
    ...
    # -- ax3: PSDF danger field --
    # use raw danger values (unnormalized) so colormap reflects intensity differences
    ...
    weight_str = (
        ...
        f"PSDF: dual-layer Proxemics Gaussian inner={PSDF_INNER_R}m(w={PSDF_INNER_W}) "
        f"outer={PSDF_OUTER_R}m(w={PSDF_OUTER_W})  "
        f"| path diversity: block_sigma={REF_BLOCK_SIGMA_M}m penalty={REF_BLOCK_WEIGHT}"
    )
    plt.suptitle(
...
def main():
    ...
    print(f"[ISLANDS] total={len(all_islands)}  valid(≥{MIN_ISLAND_AREA_THRESHOLD}m2)={len(valid_islands)}")
    for idx, isl in enumerate(valid_islands):
        cap = max(1, int(isl["area_m2"]/MIN_ISLAND_AREA_PER_PERSON))
        print(f"  island[{idx}] area={isl['area_m2']:.1f}m2  capacity={cap}")
    ...
    # actual_num = scene_area / area_per_person, capped by num_people
    area_based_num = max(1, int(nav_area / AREA_PER_PERSON_M2))
    actual_num = min(ARGS.num_people, area_based_num)
    print(f"[GEN] nav_area={nav_area:.1f}m2  AREA_PER_PERSON={AREA_PER_PERSON_M2}m2  "
    ...
    for ep_id in ARGS.episode_ids:
        ...
        # strip non-serializable 'pixels' field
