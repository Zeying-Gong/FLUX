#!/usr/bin/env python3
"""
generate_episode_fast_tracking.py  (with --easy_mode and seed‑retry)
────────────────────────────────────────────────────────────────────
Pure‑Python tracking episode generator without Isaac Sim.
...
def load_2d_map(sem_json, robot_r, scale):
    ...
    return planning_grid, esdf, min_x, min_y, scale, sem_data, raw_grid, min_y, max_y, min_x, max_x
...
def generate_one_episode(episode_id, seed, out_dir, scene_id,
    ...
    def sample_point():
        for _ in range(10):
            px_py = sample_free_point(grid, esdf, rng, PED_SPAWN_MIN_CLEARANCE_M, _free_cache)
        ...
        for d_min, d_max, n_tries in dist_passes:
            for _ in range(n_tries):
                ...
                if astar(grid, start_px, target_px_snap,
                         esdf=esdf, min_corridor_m=ARGS.robot_radius_2d*0.5,
...
def main():
    ...
    grid, esdf, min_x, min_y, scale, sem_data, raw_grid, map_min_y, map_max_y, map_min_x, map_max_x = res
    ...
    scene_id = os.path.basename(ARGS.semantic_map_json)
    m = re.match(r"2D_Semantic_Map_\d+_(\d+)_Complete\.json", scene_id)
    if m:
        scene_id = m.group(1)
    else:
        scene_id = "unknown"
    ...
    for eid in ARGS.episode_ids:
        ...
        for retry in range(max_retries):
            ...
            print(f"\n[GEN] episode_{eid} seed={seed} num_people={actual_num}" +
            ...
            if result.get("skipped") is not True:
                ...
                break
        if not success:
            print(f"  [ep{eid}] FAILED after {max_retries} attempt(s)")
